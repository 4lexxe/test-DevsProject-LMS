...
export const usePermissions = () => {
  ...
  const checkPermission = (permission: string | string[], ownerId?: number): boolean => {
  ...
  return {
    ...
    hasAnyPermission: (permissions: string[]) => checkPermission(permissions),
    ...
    canModifyResource: (resourceUserId: number) => 
      checkPermission(['manage:own_resources', 'moderate:all_resources'], resourceUserId),
    
    canModifyComment: (commentUserId: number) => 
      checkPermission(['manage:own_comments', 'moderate:all_comments'], commentUserId),
    
    canModifyRating: (ratingUserId: number) => 
      checkPermission(['manage:own_ratings', 'moderate:all_ratings'], ratingUserId),
    
    canDeleteAnyResource: () => 
      checkPermission('moderate:all_resources') || user?.role?.name === 'admin'
  };
};
